...
class _List_node_base {
 public:
  ...
  _Base_ptr self() {  //将这个_List_node_base对象的地址 转换成一个指针类型的地址
    return static_cast<_Base_ptr>(&*this);
  }
  void unlink() {  //空结点首尾指针都指向自己
    _Prev = _Next = self();
  }
  _Node_ptr
  as_node() {  //基类指针转换成派生类指针，使其可以访问派生类新增的成员
...
class _List_node : public _List_node_base<T> {
 public:
  ...
  _Base_ptr as_base() {  //派生类转换成基类指针，去除了指向_Myval的能力
    return static_cast<_Base_ptr>(&*this);
  }
};
...
void list<T>::link_node_at_back(_Base_ptr _First, _Base_ptr _Last) {
  ...
  */
  ...
  _First->_Prev->_Next =
      _First;  //如果_Node之前没有结点，这两句句的作用就是_First=_First；
  _Node->_Prev = _Last;
}
...
void list<T>::resize(size_type _Newsize, const value_type& _Val) {
  if (_Newsize > size()) {
    ...
    while (_Delete_size--) pop_back();  //注意pop_back本身也会对_Size进行减一
    _Size = _Newsize;
  }
}
...
void list<T>::remove_if(Predicate _Pred) {
  ...
  for (auto _Next = _First; _First != _Last; _First = _Next) {
    ...
    ++_Next;
    if (_Pred(*_First)) erase(_First);  // erase中有--_Size
  }
}
