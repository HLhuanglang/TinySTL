...
typename iterator_traits<_InIt>::difference_type distance(IN _InIt _First,
                                                          IN _InIt _Last) {
...
template <class _InIt>
typename iterator_traits<_InIt>::difference_type Pstar_distance(
    IN _InIt _First, IN _InIt _Last, DUMMY input_iterator_tag) {
...
typename iterator_traits<_RanIt>::difference_type Pstar_distance(
    IN _RanIt _First, IN _RanIt _Last,
    DUMMY
        random_access_iterator_tag) { /*如果不是iterator种类不是random类型，假设是farward，则会调用上面的input版本，因为farward继承自input*/
  return (_Last - _First);
}
...
template <class _InIt, class _Distance>
inline void Pstar_advance(
    IN _InIt& i, IN _Distance n,
    DUMMY
        input_iterator_tag) { /*advance算法对forward_iter和input_iterator效果是一样的，由于继承关系，可只实现input_iterator*/
  while (n--) ++i;
}

template <class _InIt, class _Distance>
inline void Pstar_advance(IN _InIt& i, IN _Distance n,
                          DUMMY bidirectional_iterator_tag) {
  if (n >= 0)
    while (n--) ++i;
  else
    while (n++) --i;
...
template <class _InIt, class _Distance>
inline void Pstar_advance(IN _InIt& i, IN _Distance n,
                          DUMMY random_access_iterator_tag) {
  i += n;
}
...
class reverse_iterator {
 ...
 public:
  ...
  self& operator+(differece_type n) const { return self(Cur - n); }
  ...
  reference operator[](differece_type n) const { return *(*this + n); }
};  // end of class reverse_iterator

//重载
template <class Iter>
typename TinySTL::reverse_iterator<Iter>::difference_type operator-(
    const reverse_iterator<Iter>& _Left, const reverse_iterator<Iter>& _Right) {
  return _Right.base() - _Left.base();
}

template <class Iter>
bool operator==(const reverse_iterator<Iter>& _Left,
                const reverse_iterator<Iter>& _Right) {
  return _Right.base() == _Left.base();
}
