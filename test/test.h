...
class TestCase {
 public:
  TestCase(const char* name) : _Case_Name(name) {}
  virtual void Run() = 0;
...
//开启所有的算法测试，容器单独设计
#define RUN_ALL_TESTS() TinySTL::test::UnitTest::GetInstance()->Run()
...
#define TEST(_Name) STL_TEST(_Name)

//将_Name替换成_Name_TEST
#define TESTCASE_NAME(_Name) _Name##_TEST
...
#define STL_TEST(_Name)                                                      \
  class TESTCASE_NAME(_Name) : public TestCase {                             \
   public:                                                                   \
    TESTCASE_NAME(_Name)(const char* _Case_Name) : _Case_Name(_Case_Name){}; \
    virtual void Run();                                                      \
                                                                             \
   private:                                                                  \
    static TestCase* const testcase_;                                        \
                                                                             ...
                                                                             \
  TestCase* const TESTCASE_NAME(_Name)::testcase_ =                          \
